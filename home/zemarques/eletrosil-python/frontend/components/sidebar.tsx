...
import { CalendarSearch, Inbox, LayoutDashboard, Search, User as UserIcon, Receipt, DollarSign, Shield, Store, Building2, LogOut, Users } from "lucide-react"
...
// ── Itens base da navegação (sem filtro de role) ──
// A lógica de permissão é aplicada dinamicamente no render
const allNavItems = [
  { href: "/dashboard", label: "Dashboard", icon: LayoutDashboard, roles: ["admin", "manager", "vendedor", "operator"] },
  { group: "Gestão", roles: ["admin", "manager"] },
  { href: "/categories", label: "Categorias", icon: Search, roles: ["admin", "manager"] },
  ...
  { href: "/customers", label: "Clientes", icon: Users, roles: ["admin", "manager", "vendedor", "operator"] },
  { group: "Estoque", roles: ["admin", "manager", "operator"] },
  { href: "/products", label: "Produtos", icon: Inbox, roles: ["admin", "manager", "vendedor", "operator"] },
  { href: "/moves", label: "Entrada/Saída", icon: CalendarSearch, roles: ["admin", "manager", "operator"] },
  { group: "Vendas", roles: ["admin", "manager", "vendedor", "operator"] },
  { href: "/caixa", label: "Caixa", icon: DollarSign, roles: ["admin", "manager", "operator"] },
  { href: "/pdv", label: "PDV", icon: CalendarSearch, roles: ["admin", "manager", "vendedor", "operator"] },
  { href: "/sales", label: "Histórico", icon: Receipt, roles: ["admin", "manager", "vendedor", "operator"] },
]

// Função para filtrar itens por role do usuário
...
export function AppSidebar({ branches = [] }: Props) {
  ...
  // Filtra navItems baseado na role do usuário
