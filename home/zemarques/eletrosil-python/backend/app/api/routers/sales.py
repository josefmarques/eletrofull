...
class PDVCheckoutPayload(BaseModel):
    ...
    branch_id: UUID
    customer_id: Optional[UUID] = None  # Cliente associado à venda
    seller_id: Optional[UUID] = None  # Vendedor responsável pela comissão
    payment_method: str  # PIX, CREDIT, DEBIT, CASH
...
def get_commission_report(
    ...
    Se nenhuma data for informada, retorna dos últimos 30 dias.
    """
    now = datetime.now()
...
def create_sale(
    ...
    """Cria uma nova venda com itens, atualiza estoque, registra movimentações e calcula comissão."""
    if not body.items:
...
def pdv_checkout(
    ...
    Agora suporta:
    - customer_id: vincula cliente à venda
    - seller_id: vincula vendedor e calcula comissão automaticamente
    """
    if not body.items:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
