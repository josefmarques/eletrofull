...
class Quote(SQLModel, table=True):
    """Modelo da tabela 'quotes' - Orçamentos/Propostas.
    
    Um orçamento é uma pré-venda: lista itens, calcula totais,
    mas NÃO baixa estoque e NÃO gera movimentação financeira.
    """
    ...
    seller_id: Optional[UUID] = Field(default=None, foreign_key="users.id")  # Vendedor responsável
    ...
    status: str = Field(default="pendente", nullable=False)  # pendente, aprovado, cancelado
    expires_at: datetime = Field(nullable=False)  # Padrão: 5 dias após criação
...
class QuoteItem(SQLModel, table=True):
    """Modelo da tabela 'quote_items' - Itens dos orçamentos."""
    ...
    product_name: Optional[str] = Field(default=None)  # Denormalizado para impressão
