...
_VALID_ROLES = Literal["admin", "manager", "operator"]
...
def create_user(
    ...
    if body.role not in ("admin", "manager", "operator"):
        raise HTTPException(
            ...
            detail=f"Role inválida: '{body.role}'. Use: admin, manager ou operator.",
        )
...
def update_user(
    ...
    if role is not None:
        if role not in ("admin", "manager", "operator"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Role inválida: '{role}'. Use: admin, manager ou operator.",
            )
        user.role = role
